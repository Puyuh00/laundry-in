...
<head>
    ...
    if (isset($_GET['id_pembayaran'])) {
        ...
        echo "ID Pembayaran: " . $_GET['id_pembayaran'];

        // Ambil data pembayaran berdasarkan id_pembayaran dari database
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            if (isset($_POST['simpan'])) {
                ...
                if (!empty($metode_pembayaran) && !empty($tanggal_pembayaran) && !empty($total_pembayaran)) {
                    // Update data pembayaran dalam database
                    $update_query = "UPDATE pembayaran SET metode_pembayaran = '$metode_pembayaran', tanggal_pembayaran = '$tanggal_pembayaran', total_pembayaran = '$total_pembayaran' WHERE id_pembayaran = '$id_pembayaran'";
                    $update_result = mysqli_query($koneksi, $update_query);

                    if ($update_result) {
                        // Set session untuk alert berhasil
                        $_SESSION['success_message'] = "Data pembayaran berhasil diedit.";

                        // Redirect ke halaman data pembayaran setelah berhasil melakukan pengeditan
                        ...
                        <div class="col-sm-12">
                            <div class="panel panel-border panel-primary">
                                <div class="panel-heading">
                                    <h3 class="panel-title ion-ios-wallet"> Edit Data Pembayaran</h3>
                                </div>
                                <div class="panel-body">
                                    <div class="form">
                                        <form method="POST" action="edit_pembayaran.php?id_pembayaran=<?= $id_pembayaran ?>" class="cmxform form-horizontal tasi-form">
                                            <div class="form-group">
                                                <label class="control-label col-lg-2" for="metode_pembayaran"> Metode Pembayaran :</label>
                                                <div class="col-lg-5">
                                                    <input type="text" class="form-control" name="metode_pembayaran" value="<?= $row['metode_pembayaran'] ?>" required>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="control-label col-lg-2" for="tanggal_pembayaran"> Tanggal Pembayaran :</label>
                                                <div class="col-lg-5">
                                                    <input type="text" class="form-control" name="tanggal_pembayaran" value="<?= $row['tanggal_pembayaran'] ?>" required>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="control-label col-lg-2" for="total_pembayaran"> Total Pembayaran :</label>
                                                <div class="col-lg-5">
                                                    <input type="text" class="form-control" name="total_pembayaran" value="<?= $row['total_pembayaran'] ?>" required>
                                                </div>
                                            </div>
                                            <div class="form-group">
