...
$query = "SELECT TRIM(id_pelanggan) AS id_pelanggan FROM pelanggan ORDER BY id_pelanggan DESC LIMIT 1";
$result = mysqli_query($koneksi, $query);
if (mysqli_num_rows($result) > 0) {
  $row = mysqli_fetch_assoc($result);
  $lastCustID = $row['id_pelanggan'];

  // Menghasilkan ID pelanggan berikutnya
  $nextCustID = generateNextCustID($lastCustID);
  
} else {
  // Jika belum ada data pelanggan, menghasilkan ID pelanggan awal
  $nextCustID = "C001";
}

// Fungsi untuk menghasilkan ID pelanggan berikutnya
function generateNextCustID($lastCustID)
{
  // Mengambil angka dari ID pelanggan terakhir
  $lastNumber = (int) substr($lastCustID, 1);

  // Menambahkan 1 ke angka terakhir
  $nextNumber = $lastNumber + 1;

  // Menghasilkan ID pelanggan berikutnya dengan format CXXX
  $nextCustID = "C" . str_pad($nextNumber, 3, "0", STR_PAD_LEFT);
              ...
              <div class="panel panel-border panel-dark">
                <div class="panel-heading">
                  <div class="pull-right">
                    <a href="data_pelanggan.php" class="btn btn-success ion-ios-person"> Data Pelanggan</a>
                  </div>
                  <h3 class="panel-title ion-ios-person"> Pelanggan</h3>
                </div>
                <div class="panel-body">
                  <div class="form">
                    <form method="POST" action="detail_pelanggan.php" enctype="multipart/form-data" class="cmxform form-horizontal tasi-form" id="commentForm">
                      <div class="form-group">
                        <label class="control-label col-lg-2">ID Pelanggan :</label>
                        <div class="col-lg-5">
                          <input type="text" class="form-control" name="id_pelanggan" id="pelanggan" value="<?php echo $nextCustID; ?>" readonly>
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-lg-2">Nama Pelanggan :</label>
                        <div class="col-lg-5">
                          <input type="text" class="form-control" name="nama_pelanggan" id="nama" placeholder="Masukan Nama Pelanggan" required autocomplete="off">
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-lg-2">Alamat :</label>
                        <div class="col-lg-5">
                          <input type="text" class="form-control" name="alamat" placeholder="Masukan Alamat" id="alamat" autocomplete="off">
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-lg-2">No.Telepon :</label>
                        <div class="col-lg-5">
                          <input type="text" class="form-control" name="no_telepon" id="bayar" placeholder="Masukan No.Telepon" required autocomplete="off">
                        </div>
                      </div><br>
                      <div class="form-group">
                        <div class="col-lg-offset-5 col-lg-10">
                          <input type="submit" name="kirim" value="Tambah Pelanggan" class="btn-primary btn-rounded">
                        </div>
                      </div>
                    </form>
